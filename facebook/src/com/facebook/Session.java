...
public class Session implements Serializable {
    ...
    private Session(String applicationId, SessionState state,
        ...
        this.tokenInfo = tokenInfo;
        this.lastAttemptedTokenExtendDate = lastAttemptedTokenExtendDate;
        this.pendingRequest = pendingAuthorizationRequest;
    ...
    public final boolean onActivityResult(Activity currentActivity, int requestCode, int resultCode, Intent data) {
        ...
        finishAuthOrReauth(null,null, null,exception); 
        return true;
    }
    ...
    void authorize(AuthorizationRequest request) {
        ...
        pendingRequest.loggingExtras.put(AuthorizationClient.EVENT_EXTRAS_TRY_LOGIN_ACTIVITY,
                started ? AppEventsConstants.EVENT_PARAM_VALUE_YES : AppEventsConstants.EVENT_PARAM_VALUE_NO);

        if (!started && request.isLegacy) {
            pendingRequest.loggingExtras.put(AuthorizationClient.EVENT_EXTRAS_TRY_LEGACY,
    ...
    private void handleAuthorizationResult(int resultCode, AuthorizationClient.Result result) {
        ...
        finishAuthOrReauth(authCode, newToken, loggingExtras, exception);        
        //finishAuthOrReauth(newToken, exception);
    }

    private void logAuthorizationStart() {
        Bundle bundle = AuthorizationClient.newAuthorizationLoggingBundle(pendingRequest.getAuthId());
        ...
        try {
            ...
            extras.put(AuthorizationClient.EVENT_EXTRAS_LOGIN_BEHAVIOR,
                    pendingRequest.loginBehavior.toString());
            extras.put(AuthorizationClient.EVENT_EXTRAS_REQUEST_CODE, pendingRequest.requestCode);
            extras.put(AuthorizationClient.EVENT_EXTRAS_IS_LEGACY, pendingRequest.isLegacy);
            extras.put(AuthorizationClient.EVENT_EXTRAS_PERMISSIONS,
                    TextUtils.join(",", pendingRequest.permissions));
            extras.put(AuthorizationClient.EVENT_EXTRAS_DEFAULT_AUDIENCE,
                    pendingRequest.defaultAudience.toString());
    ...
    private void logAuthorizationComplete(AuthorizationClient.Result.Code result, Map<String, String> resultExtras,
        ...
        if (pendingRequest == null) {
            ...
            bundle = AuthorizationClient.newAuthorizationLoggingBundle(pendingRequest.getAuthId());
            if (result != null) {
            ...
            if (pendingRequest.loggingExtras.isEmpty() == false) {
                jsonObject = new JSONObject(pendingRequest.loggingExtras);
            }
...
//                    permissions, defaultAudience, applicationId, validateSameFbidAsToken, delegate);
            return new AuthorizationClient.AuthorizationRequest(loginBehavior, requestCode, isLegacy,
                    permissions, defaultAudience, applicationId, redirectUri, validateSameFbidAsToken, delegate, authId);
        }
