        ...
        void onWebDialogComplete(AuthorizationRequest request, Bundle values,
            ...
            if (values != null) {
                ...
                if(values.getString(AUTH_CODE_KEY) != null) {
                	outcome = Result.createCodeBundleResult(request,values.getString(AUTH_CODE_KEY),values);               	
                }
                else if(token!=null 
                		&& values.getString(SESSION_KEY_KEY)!=null) {
                	outcome = Result.createTokenBundleResult(request,token, values);                  
                }
                else 
                {
...
//        private Result(Code code, AccessToken token, String errorMessage) {
        private Result(AuthorizationRequest request, Code code, String authCode, AccessToken token, 
            String errorMessage, String errorCode, Bundle bundle) {        
            ...
            for (String key : bundle.keySet()) {
            	loggingExtras.put(key, bundle.getString(key));
            }    
        }

        static Result createTokenResult(AuthorizationRequest request,AccessToken token) {
            //return new Result(Code.SUCCESS, token, null);
            return new Result(request, Code.SUCCESS, null,token, null, null, null);        	
        }

        static Result createCodeResult(AuthorizationRequest request,String authCode) {
            return new Result(request, Code.SUCCESS, authCode, null, null, null, null);
        }      
               
        static Result createCodeBundleResult(AuthorizationRequest request,String authCode, Bundle bundle) {
            return new Result(request, Code.SUCCESS, authCode, null, null, null, bundle);
        }   
 
        static Result createTokenBundleResult(AuthorizationRequest request,AccessToken token, Bundle bundle) {
            return new Result(request, Code.SUCCESS, null, token,  null, null, bundle);
        }              
        
        static Result createCancelResult(AuthorizationRequest request,String message) {
            //return new Result(Code.CANCEL, null, message);
            return new Result(request, Code.CANCEL, null, null, message, null, null);        	
        }
        ...
        static Result createErrorResult(AuthorizationRequest request, String errorType, String errorDescription) {
            ...
            return new Result(request, Code.ERROR, null, null, message, null, null);  
        }

        static Result createErrorResult(AuthorizationRequest request, String errorType, String errorDescription,
                String errorCode) {
            String message = TextUtils.join(": ", Utility.asListNoNulls(errorType, errorDescription));
            return new Result(request, Code.ERROR, null, null, message, errorCode, null);
        }
    }
}
